...
#                           (OLD/superseded: Conv2d-mixed noise + learnable scale --
#                           kept only for historical comparison, not resubmitted here)
...
#
#   FOSI_enscale_sigma<S>    -- fixed single-shot EnScale refiner (--stochastic-refine),
#                               now using a true per-location LocallyConnected2d noise
#                               mixing layer + shared MLP instead of FOSI_enscale_lite's
#                               Conv2d + learnable scale. S in {0, 0.5, 1, 2, 4} is the
#                               fixed (non-learnable) noise_sigma dial.
#   FOSI_enscalenet_sigma<S> -- full EnScale decoder (--enscale-net): the same mechanism
#                               applied progressively at every 2x upsampling stage. S in
#                               {0.5, 1, 2} (smaller sweep -- bigger architecture change,
#                               confirm it trains well before a wider sweep).
#
# Usage:
...
#
# Submits 40 jobs from the original 8 arms, plus 40 more from the 8 new
# EnScale-sigma arms (5 splits x (5 fixed-refiner + 3 EnScaleNet) sigma values)
# = 80 jobs total across both sections below. Each is a 1-GPU v100 Casper job,
# walltime up to 6h (see submit_engressnet.sh's #PBS -l walltime).
# ==============================================================
...
for split in "${SPLITS[@]}"; do
    ...
    # ---- (6) Fixed EnScale-lite refiner: noise_sigma sweep ----
    # Supersedes FOSI_enscale_lite (Conv2d + learnable scale) with the paper-faithful
    # LocallyConnected2d mixing + shared MLP, and a real fixed dial on the noise's
    # raw magnitude instead of a learnable scalar that training could (and did) drift
    # away from its init regardless of the "dial" setting.
    for sigma in 0 0.5 1 2 4; do
        tag="FOSI_enscale_sigma${sigma}"
        submit_job "$tag" \
            "BATCH_NAME=${tag},TRAIN_YEARS=${train_years},TEST_YEARS=${TEST_YEARS},NUM_EPOCHS=${BASE_NUM_EPOCHS},K=${BASE_K},K_EVAL=${BASE_K_EVAL},BETA=${BASE_BETA},LAT_MIN=${BASE_LAT_MIN},LAT_MAX=${BASE_LAT_MAX},LON_MIN=${BASE_LON_MIN},LON_MAX=${BASE_LON_MAX},STOCHASTIC_REFINE=true,NOISE_SIGMA=${sigma}"
    done

    # ---- (7) EnScaleNet (full progressive decoder): noise_sigma sweep ----
    for sigma in 0.5 1 2; do
        tag="FOSI_enscalenet_sigma${sigma}"
        submit_job "$tag" \
            "BATCH_NAME=${tag},TRAIN_YEARS=${train_years},TEST_YEARS=${TEST_YEARS},NUM_EPOCHS=${BASE_NUM_EPOCHS},K=${BASE_K},K_EVAL=${BASE_K_EVAL},BETA=${BASE_BETA},LAT_MIN=${BASE_LAT_MIN},LAT_MAX=${BASE_LAT_MAX},LON_MIN=${BASE_LON_MIN},LON_MAX=${BASE_LON_MAX},ENSCALE_NET=true,NOISE_SIGMA=${sigma}"
    done
done
