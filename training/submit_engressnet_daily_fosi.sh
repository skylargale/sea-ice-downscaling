...
# Pretraining / transfer learning: set INIT_CHECKPOINT to a prior run's model_state_dict.pt
# (e.g. a MESA-trained checkpoint) to initialize this run's weights from it instead of random
# init. FOSI's X has 4 predictor channels (vector wind, u_10/v_10) vs. MESA's 3 (wind speed
# only), so set COLLAPSE_WIND_VECTOR=true when initializing from a MESA checkpoint -- otherwise
# the state_dict load fails on the first conv layer's channel count. Leave both blank (default)
# for normal random-init training. See train_engressnet.py --init-checkpoint/--collapse-wind-vector.
