...
# cd to the Version5 root (not $PBS_O_WORKDIR, which varies by invocation convention --
# see submit/README.md): --save-root defaults to the relative path "saved_figs", and
# submit/evaluation/ has no saved_figs symlink (only results/), so without this a run
# could silently create a stray submit/evaluation/saved_figs/ instead of writing into the
# real Version5/saved_figs/. Hardcoded absolute path, not resolved relative to
