...
def load_and_prepare(config):
    ...
    if getattr(config, "coastal_channel", False):
        raise NotImplementedError("Analog baseline not wired up for coastal_channel=True.")
...
def build_analog_ensemble(d, k):
    ...
    Y_train_phys = (d["Y_train"] * d["Y_std"] + d["Y_mean"]).clamp(min=0.0)  # (N_train, C, H, W)
    ...
    sit_idx = 0
    X_test_sit_phys = d["X_test"][:, sit_idx:sit_idx + 1] * d["X_std"][:, sit_idx:sit_idx + 1] + d["X_mean"][:, sit_idx:sit_idx + 1]
    Y_base_phys = F.interpolate(X_test_sit_phys, size=d["Y_test"].shape[-2:], mode="bilinear", align_corners=False)

    Y_test_phys = d["Y_test"] * d["Y_std"] + d["Y_mean"]
