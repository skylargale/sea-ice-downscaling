...
def ice_edge_error(pred, truth, threshold=0.0, mask_bool=None, lat=None):
    """Kept in sync with training/functions_engressnet.py's ice_edge_error -- see its
    docstring for the cos(lat) area-weighting rationale."""
    ice_pred = pred[:, 0] > threshold
    ice_truth = truth[:, 0] > threshold
    disagree = (ice_pred != ice_truth).float()
...
def compute_member_avg_metrics(preds_all_phys, truth_phys, mask_test=None, coastal_width=5, hlat=None):
    """
    ...
    return {
        ...
        "IIEE": avg(ice_edge_error, mask_bool=ocean_bool, lat=hlat),
