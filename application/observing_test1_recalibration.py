"""
observing_test1_recalibration.py

Test 1 (post-hoc uncertainty recalibration): fits a single scalar spread
multiplier `s` -- scaled_member = ensemble_mean + s*(member - ensemble_mean)
-- so the network's 5-95% ensemble band actually covers ~90% of real
CryoSat-2 truth, instead of the ~1.5-2.8% measured with the raw (unscaled)
ensemble. Fit on Jan-Feb 2020 (land-fixed run), validated on held-out
Mar-Apr 2020, coastal and interior separately (they need very different
multipliers given how differently miscalibrated they are).

This does not touch point accuracy (the ensemble mean is unchanged) --
it only widens/narrows the reported uncertainty band to be honest about
real-world error, which is a coverage/calibration fix, not an
RMSE-vs-bilinear fix.
"""
...
for region_name, region_mask in [("coastal", coastal_hw), ("interior", interior_ocean_hw)]:
    ...
    for s in [1, 2, 4, 6, 8, 10, 15, 20, 25, 30, 40, 50, 70, 100, 150, 200]:
